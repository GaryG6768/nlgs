...
(function(){
  ...
  console.log('NLGS FRIENDLY PERSONAL SCORING ORDER V3 ACTIVE');
  ...
  /* Keep the chosen order applied whenever the scoring rows are
     rebuilt by the main app (for example when changing holes). */
  const holeObserver = new MutationObserver(() => {
    try{
      const r = game();
      if(!r) return;
      const saved = readSavedOrder(r);
      if(saved && confirmed[gameKey(r)]){
        applyOrder(r, saved);
      }
    }catch(e){}
  });
  ...
  setTimeout(() => {
    const holder = document.getElementById('friendlyHolePlayers');
    if(holder){
      holeObserver.observe(holder, {childList:true});
    }
  }, 200);
