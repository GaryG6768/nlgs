...
function holeStatus(game,h){
  ...
  */
  ...
  // Use Chrome's native View Transition when available. Otherwise render
  // immediately rather than using an opacity fade, which can look like a flash.
  function smoothFriendlyRender(){
    if(typeof originalRender!=='function') return;
    try{
      if(document.startViewTransition){
        document.startViewTransition(function(){ originalRender(); });
      }else{
        originalRender();
      }
    }catch(e){
