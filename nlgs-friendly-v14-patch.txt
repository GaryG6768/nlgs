...
(function(){
  console.log('NLGS FRIENDLY MULTI-SCORER TEST V12 ACTIVE');
...
function holeStatus(game,h){
  ...
  */
  ...
  async function refreshStatus(){
    ...
    multiStatus=r.data;syncScoreCards();syncLeaderboard();syncHoleGrid();ensureAgreement();return r.data;
  }
  ...
  window.nlgsFinaliseFriendlyRound=async function(){
    ...
    try{
      const game=round();await refreshStatus();
      for(let h=1;h<=18;h++){if(!holeStatus(game,h).agreed){if(typeof toast==='function')toast('Round cannot be finalised until all four scorers agree every hole.');return;}}
      game.scores={};
      game.players.forEach(function(player,index){
        game.scores[player]={};
        for(let h=1;h<=18;h++){const v=agreedScore(game,index,h);if(v===null)throw new Error('A final agreed score could not be established.');game.scores[player][h]=v;}
      });
      if(typeof saveFriendlyState==='function')saveFriendlyState();
    ...
    }catch(e){console.error(e);if(typeof toast==='function')toast(e.message||'Could not finalise the round');}
    finally{finalising=false;}
  };
...
// V14 safety: recover/create the Supabase Friendly Game record when an
