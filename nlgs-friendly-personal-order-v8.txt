...
(function(){
  ...
  console.log('NLGS FRIENDLY PERSONAL SCORING ORDER V8 ACTIVE');
  ...
  /* Re-apply the scorer's chosen visual order whenever the normal
     Friendly renderer rebuilds the hole cards. This keeps the order
     across Hole 1 -> Hole 2 -> Hole 3 etc. without changing indexes. */
  ...
  window.renderFriendlyScore = async function(){
    ...
    setTimeout(()=>{
      try{
        const r=game();
        if(r && chosenGameId === gameKey(r) && Array.isArray(chosenOrder)){
