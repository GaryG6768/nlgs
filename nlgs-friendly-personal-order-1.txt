...
(function(){
  ...
  console.log('NLGS FRIENDLY PERSONAL SCORING ORDER ACTIVE');
  ...
  function makeOrderCard(r, initialOrder){
    ...
    function render(){
      card.innerHTML = `
        ...
        <button id="nlgsOrderConfirm" class="btn" style="margin-top:14px">
          START SCORING
        </button>
      `;
      ...
      card.querySelector('#nlgsOrderConfirm').onclick = () => {
        saveOrder(r, order);
        confirmed[gameKey(r)] = true;
        showScoring();

        /* Re-render through V15 so all of its live-scoring behaviour
           remains intact, then apply this scorer's display order. */
        if(typeof window.renderFriendlyScore === 'function'){
          window.renderFriendlyScore();
        }
        setTimeout(() => applyOrder(r, order), 40);
  ...
  function maybeShowOrder(){
    ...
    /* Once this scorer has chosen an order for this game, never ask
       again on every hole/render. */
    if(saved){
      confirmed[key] = true;
      return;
    }
    ...
    /* Let the existing renderer create the normal scoring rows first. */
    const root = scoringArea();
    if(!root) return;
  ...
  function applySavedOrderAfterRender(){
    const r = game();
    if(!r || gameKey(r) !== currentGameId) return;

    const saved = readSavedOrder(r);
    if(saved && confirmed[currentGameId]){
      setTimeout(() => applyOrder(r, saved), 20);
    }
  }
  ...
  /* If the app changes screens into Friendly Scoring without calling
     renderFriendlyScore immediately, catch the screen becoming active. */
