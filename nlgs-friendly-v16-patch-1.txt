...
function holeStatus(game,h){
  ...
  */
  ...
  // Smooth the Friendly Game screen when moving to the next hole.
  // The existing renderer is still used, but the short fade prevents the
  // visible flash caused by rebuilding the score cards after every save.
  function smoothFriendlyRender(){
    const root=document.getElementById('friendlyScore');
    if(!root || typeof originalRender!=='function'){
