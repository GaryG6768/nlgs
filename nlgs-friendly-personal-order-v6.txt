...
(function(){
  ...
  console.log('NLGS FRIENDLY PERSONAL SCORING ORDER V6 ACTIVE');
  ...
  /* Keep the selected order applied whenever the main scoring renderer
     rebuilds the player cards (for example after saving Hole 1). */
  let playerListObserver = null;

  function watchPlayerList(){
    const holder = document.getElementById('friendlyHolePlayers');
    if(!holder || playerListObserver) return;

    playerListObserver = new MutationObserver(() => {
      const r = game();
      if(!r) return;

      if(chosenGameId === gameKey(r) && Array.isArray(chosenOrder)){
        const rows = [...holder.children];
        if(rows.length === r.players.length){
          const current = rows.map(row => Number(
            row.querySelector('.friendly-hole-score')?.dataset.player
          ));
          const same = current.length === chosenOrder.length &&
            current.every((v,i) => v === chosenOrder[i]);

          if(!same) applyOrder(r, chosenOrder);
        }
      }
    });

    playerListObserver.observe(holder, {childList:true});
  }
